...
import faiss
import numpy as np
from config import HF_MODEL_NAME, LLM_MODEL_URL, DATABASE_PATH, HUGGINGFACE_API_KEY
...
# Add available LLM models to the configuration
AVAILABLE_LLMS = {
    "Mistral 7B (Recommended)": "mistralai/Mistral-7B-Instruct-v0.1",
    "Falcon 7B (Decent)": "tiiuae/falcon-7b-instruct",
    "GPT-2 (Unreliable)": "gpt2",
    "FLAN-T5 Large (Unreliable)": "google/flan-t5-large",
}

def update_config_with_comments(selected_llm):
    """
    Updates the config.py file to reflect the selected LLM by uncommenting the corresponding line
    and commenting out all others.
    """
    ...
    try:
        ...
        # Update the HF_MODEL_NAME section
        updated_lines = []
        for line in lines:
            if line.strip().startswith("HF_MODEL_NAME") or line.strip().startswith("# HF_MODEL_NAME"):
                # Check if the current line matches the selected LLM
                if f"\"{selected_llm}\"" in line:
                    # Uncomment the selected LLM
                    updated_lines.append(f"HF_MODEL_NAME = \"{selected_llm}\"\n")
                else:
                    # Comment out other LLMs
                    updated_lines.append(f"# {line.lstrip('#').strip()}\n")
            else:
        ...
        st.success(f"Configuration updated to use {selected_llm}. Please reload the app to apply changes.")
    except Exception as e:
    ...
    # Chat with NEXA Page
        ...
        try:
            selected_llm = st.selectbox(
                ...
                index=list(AVAILABLE_LLMS.values()).index(HF_MODEL_NAME) if HF_MODEL_NAME in AVAILABLE_LLMS.values() else 0
            )
        except (NameError, ValueError) as e:
            logger.error(f"Error with HF_MODEL_NAME: {e}")
            st.error("The current LLM model is not valid. Please select a valid model.")
            selected_llm = st.selectbox(
                "Select the Language Model:",
                options=list(AVAILABLE_LLMS.keys()),
                index=0  # Default to the first model if there's an error
            )

        # Update the config file when the user selects a new LLM
        if st.button("Update LLM"):
            update_config_with_comments(AVAILABLE_LLMS[selected_llm])
